...
project is the complete reference implementation; CacheLikesFromTwitter adapts the
...
CPU `Auto` and `CPU` execution parallelizes independent causal walk-forward
origins through `app/infrastructure/parallel.py`. Large workloads use a
...
remain serial because their ordering is a causal/accounting invariant. GPU
execution remains serialized at the origin level because the selected
