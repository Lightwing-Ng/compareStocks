...
Signal strategies may return a JSON-safe `StrategySignalResult.presentation` dictionary. The backtest engine validates finite numbers and requires any presentation `data_keys` to match `chart.raw_dates` exactly before transmitting the declarative payload; strategy-owned HTML and executable code are never accepted. `bayesian-price-field/v1` supplies aligned predictive log-return means and scales plus the 36-column, six-row-up, six-row-down geometry contract to `probability-grid-v1`. The renderer derives one stable daily step from the median positive Chart.js point spacing across the complete rendered series. Each column slot, including its following gap, is an integer multiple of that step and at least one trading day; the cell is the slot width minus the gap. The preferred field width is one quarter of the price plot, but integer-day quantization, 36 columns, and the 4 px minimum cell size take priority when those constraints require a wider field. Rows use the live Y scale, so every 1:1 square maps to an exact future-time and price interval without a cumulative offset. The 12-row field is vertically centered on the horizontal price guide, uses 2 px cell radii, and places those cells inside a 10 px outer radius with the uniquely concentric 8 px padding.

The probability field is not a Frosted Glass consumer. Its strategy-private material uses a 90%-transparent background, which is `alpha: 0.10` under the CSS alpha convention, and explicitly disables background images, blur, borders, and shadows. Standard Frosted Glass tokens and every other material consumer remain unchanged. Cell opacity is the absolute posterior probability mass for the mapped interval, not a visual renormalization against the strongest cell, and is not multiplied by the field background transparency. The field always remains to the right of its vertical guide and retains one stable width while the pointer moves. If its right edge exceeds the visible chart stack, the stack creates only the missing horizontal extent and reuses Motion Core's bouncy spring to advance `scrollLeft` by that minimum distance. A bottom scrollbar is present only while the field needs that extent; returning to a fitting point, hiding the field, clearing the pinned state, or destroying the controller springs back to zero and removes the temporary extent. Content-space coordinates add the current `scrollLeft` before browser scrolling, preserving the crosshair, guide, field, and price-cell mapping without an offset.

For vertical containment, the price scale derives top and bottom padding separately from the actual responsive grid height and the space above and below the price plot inside the complete chart stack. This keeps an extreme field centered without the former narrow-screen failure mode in which a theoretical maximum symmetric pad could collapse the entire price curve. At widths up to 767 px, the normalized presentation marks the result stack with `has-probability-field`; the shared result/history splitter measures `.trade-chart-stack`, reserves its 254 px stage minimum, and raises the complete result stack to a 600 px minimum. This preserves independent report, resizer, and history grid rows on a fresh narrow load rather than relying on a desktop split ratio. Pointer movement is coalesced to the next animation frame and tracks the curve; only a point with a finite prediction can be pinned. Blank-space clicks and Escape clear it, while probability and standard summary overlays retain their source chart and recompute after chart, viewport, or sidebar layout changes.
