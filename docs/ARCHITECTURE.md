...
Documentation version: `v1.69.14`
...
rendered price curve's right endpoint is the field's left content boundary
and is aligned with the vertical guide; the field's first column therefore
remains to the right of that guide. The horizontal guide continues through
the probability field to its right edge and is painted above the cells, while
the field's green/up rows remain above that guide and its red/down rows remain
below it, so panning cannot make the field cross or detach from the crosshair.
The terminal hover boundary is the rightmost finite price-curve point in the
same logical content space; the field's visual continuation never becomes an
additional hover domain, and a pointer beyond that curve endpoint clears both
guides and the floating field. Leaving the chart stack also clears the
pointer-tracking state, except when the pointer enters the native scroll rail
to preserve manual field scrolling.
