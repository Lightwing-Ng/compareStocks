...
} from './investment/data-utils.js?v=investment-data-utils-v1.62.0';
import {
...
} from './investment/import-feedback.js?v=investment-import-feedback-v1.3.0';
import {
...
} from './investment/stock-details.js?v=investment-stock-details-v0.8.3';
import {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const schwabPositionsCsvInput = document.getElementById('schwab_positions_csv');
    const schwabPositionsCsvStatus = document.getElementById('schwab_positions_csv_status');
    ...
    const INVESTMENT_LOADING_MODAL_ICON_CLASS = 'suggestion-loading-spinner';
    ...
    const INVESTMENT_SHARE_RENDER_MODAL_ICON_CLASS = 'suggestion-loading-spinner';
    ...
    const investmentBrokerSummarySelector = document.getElementById('investment_broker_summary_selector');
    ...
    const FUNDING_METRIC_DEFINITIONS = [
        {
            ...
            summary: 'USD deposits still available to fund investments after matched USD conversions. Deposit rows consumed by a later conversion are excluded.',
        ...
        },
        {
            ...
            summary: 'USD received from matched FX conversions after conversion commissions are deducted.',
        ...
        },
        {
            ...
            summary: 'Funding loss on matched FX conversions: conversion commission plus any deposit-to-USD shortfall. Ordinary trading P&L is excluded.',
        ...
        },
        {
            ...
            summary: 'Direct USD deposits plus net USD received from matched FX conversions. This is the funding base used by the investment ledger.',
        ...
        },
        {
            ...
            summary: 'All commissions charged by imported investment activity, including trading commissions and other broker-reported fees.',
        ...
        },
        {
            ...
            summary: 'Debit interest charged by the broker and deducted from cash. It is kept separate from commission and trading P&L.',
    ...
    const BROKER_BENEFIT_METRIC_DEFINITIONS = [
        {
            ...
            summary: 'Imported broker coupon and fee-rebate benefits whose source notional is denominated in HKD. The converted contribution is included in P&L once.',
        ...
        },
        {
            ...
            summary: 'Imported broker coupon and fee-rebate benefits whose source notional is denominated in USD. The benefit is included in P&L once.',
        ...
        },
        {
            ...
            summary: 'Imported HKD cash cards, stock-cash coupons, task rewards, and signup or funding rewards. The converted contribution is included in P&L once.',
        ...
        },
        {
            ...
            summary: 'Imported USD cash cards, stock-cash coupons, task rewards, and signup or funding rewards. The benefit is included in P&L once.',
    ...
    const OFFSHORE_GAIN_METRIC_DEFINITIONS = [
        {
            ...
            summary: 'Offshore gain view: Holdings realized P&L plus Holdings unrealized P&L, including converted broker coupon, cash, and KOL rewards. Stock-grant P&L is included once through ticker Holdings.',
    ...
    const HOLDINGS_SUMMARY_METRIC_DEFINITIONS = [
        ...
        },
        {
            key: 'realized-pnl',
            label: 'Realized P&L',
            summary: 'Realized P&L from Holdings activity plus converted broker coupon, cash, and KOL rewards. Stock-grant sale proceeds remain included through the ticker result.',
    ...
    const GENERIC_XLSX_INVESTMENT_BROKERS = new Set([
        'zircon_hk',
        'standard_xlsx',
        'cmb_cn',
        'boc_cn',
    ...
    function isInvestmentInternalTransferSourceCandidate(txn) {
        ...
        if (getInvestmentInternalTransferKind(txn) === 'security') {
            return Math.abs(Number(getTransactionQuantity(txn)) || 0) > 1e-9;
        }
    ...
    function isInvestmentInternalTransferTargetCandidateForDirection(sourceTxn, targetTxn, direction) {
        const brokerCode = normalizeInvestmentBroker(getTransactionBrokerCode(targetTxn));
        if (direction === 'security_broker_to_broker') {
            return (
                brokerCode !== normalizeInvestmentBroker(getTransactionBrokerCode(sourceTxn))
                && getNormalizedTransactionType(targetTxn) === 'transfer_in'
                && getInvestmentCanonicalTicker(targetTxn?.ticker)
                    === getInvestmentCanonicalTicker(sourceTxn?.ticker)
    ...
    function formatInvestmentInternalTransferFeeNote(feeAmount, currency) {
        ...
        return `Transfer fee ${formatInvestmentInternalTransferFeeAmount(numericFeeAmount, currency || 'USD')} is retained as broker commission.`;
    }
    ...
    function buildInvestmentInternalTransferContext(processedTransactions = []) {
        ...
        processedTransactions.forEach((txn) => {
            ...
            const normalizedType = getNormalizedTransactionType(txn);
            if (
                (normalizedType === 'withdrawal' && Math.abs(Number(getTransactionAmount(txn)) || 0) > 1e-9)
                || (normalizedType === 'transfer_in' && Math.abs(Number(getTransactionQuantity(txn)) || 0) > 1e-9)
            ) {
        ...
        sourceTransactions.forEach((sourceTxn) => {
            ...
            const automaticSecurityTarget = (
                transferKind === 'security'
                && !selectedTarget
                && options.length === 1
                && options[0].dayDistance === 0
                && options[0].amountDiff <= quantityTolerance
            ) ? options[0].targetTxn : null;
            const resolvedTarget = selectedTarget || automaticSecurityTarget;
            const resolvedTargetKey = selectedTargetKey || String(
                automaticSecurityTarget?.manual_internal_transfer_key || ''
            ).trim();
            ...
            if (resolvedTarget) {
                ...
                resolvedBindingsBySourceKey.set(sourceKey, {
                    ...
                    autoMatched: Boolean(automaticSecurityTarget),
                    ...
                    feeNote: formatInvestmentInternalTransferFeeNote(
                        feeAmount,
                        formatTransactionCurrency(sourceTxn) || formatTransactionCurrency(resolvedTarget) || 'USD'
                    ),
                });
            }
        });
    ...
    function resolveInvestmentLogoUrl(profile, ticker) {
        const moneyMarketFundLogoUrl = getMoneyMarketFundLogoUrl(ticker);
    ...
    function setImportFeedback(message, variant = 'success', { allowHtml = false } = {}) {
                ...
                const feedbackTitle = isLoading
                    ...
                    : isError
                        ...
                        : isWarning
                            ? 'Import warning'
                            : 'Investment update';
                importFeedbackMessage.innerHTML = `
                    <p class="notice-floating-banner-heading">${feedbackTitle}</p>
                    <p class="notice-floating-banner-copy">${escapeHtml(resolvedMessage)}</p>
                `.trim();
            }
        } else {
        ...
        if (importFeedbackIcon) {
            importFeedbackIcon.classList.toggle('notice-floating-banner-icon-error', isError || isWarning);
            importFeedbackIcon.classList.toggle('notice-floating-banner-icon-success', !isError && !isWarning && !isLoading);
            importFeedbackIcon.classList.toggle('icon-modal-dialog-banner-default', isError || isWarning || isLoading);
            importFeedbackIcon.classList.toggle('suggestion-loading-spinner', isLoading);
        }
    }
    ...
    function clearImportFeedback() {
        ...
        if (importFeedbackIcon) {
            importFeedbackIcon.classList.remove('notice-floating-banner-icon-error');
            importFeedbackIcon.classList.remove('notice-floating-banner-icon-success');
            importFeedbackIcon.classList.remove('suggestion-loading-spinner');
    ...
    function isDollarTokenMoneyMarketTicker(ticker) {
        return (
            ...
            && getTickerQuoteCurrency(ticker) === 'USD'
        );
    }
    ...
    function getMoneyMarketFundLogoUrl(ticker) {
        if (!isMoneyMarketFundTicker(ticker)) return '';
        return isDollarTokenMoneyMarketTicker(ticker)
    ...
    function getCashEquivalentFundDisplayName(ticker) {
        ...
        if (!isLongbridgeHkCashEquivalentSyntheticTicker(normalizedTicker)) {
            ...
            if (isMoneyMarketFundTicker(normalizedTicker)) return 'Money Market Fund';
            return '';
        }
    ...
    function getInvestmentBrokerSummarySelectedCode() {
        ...
        const normalizedCurrentCode = normalizeInvestmentBroker(investmentBrokerSummarySelectedCode);
        if (normalizedCurrentCode === 'all') {
            investmentBrokerSummarySelectedCode = 'all';
            return 'all';
        }
        if (availableBrokerCodes.includes(normalizedCurrentCode)) {
            investmentBrokerSummarySelectedCode = normalizedCurrentCode;
    ...
    function syncInvestmentBrokerFilterTrigger(field) {
        ...
        if (isInvestmentBrokerSummaryFilterField(field)) {
            ...
            const selectedLabel = selectedCode === 'all'
                ? 'All'
                : getInvestmentBrokerMeta(selectedCode).label;
    ...
    function applyInvestmentBrokerSummarySelection(nextBrokerCode, field = null) {
        ...
        const normalizedBrokerCode = normalizeInvestmentBroker(nextBrokerCode);
    ...
    function mountInvestmentBrokerSummarySelector() {
        if (!(investmentBrokerSummarySelector instanceof HTMLElement)) return;
        ensureInvestmentBrokerFilterDocumentListeners();
        if (!investmentBrokerSummarySelector.querySelector('[data-investment-broker-filter]')) {
            investmentBrokerSummarySelector.innerHTML = renderInvestmentBrokerFilterHeaderInnerMarkup(
                'investment_broker_summary_filter',
                { singleSelect: true },
            );
        }
        const field = investmentBrokerSummarySelector.querySelector('[data-investment-broker-filter]');
    ...
    function mountInvestmentSideFilterHeaders(root = document) {
        if (!investmentSideFilterDocumentListenersBound) {
            investmentSideFilterDocumentListenersBound = true;
            ...
            <div class="field investment-currency-filter-field backtest-shared-select-field"
                ...
                <div class="trade-strategy-row backtest-shared-select-row investment-currency-filter-row">
                    <button type="button"
                        ...
                        <span class="trade-strategy-trigger-label" data-investment-currency-filter-label>${escapeHtml(selectedLabel)}</span>
                        <span class="trade-strategy-trigger-chevron" aria-hidden="true"></span>
                    </button>
    ...
    function openInvestmentCurrencyFilterDropdown(field) {
        ...
        dropdown.innerHTML = ['all', ...availableCurrencies].map((value) => {
            ...
            const selected = value === investmentCurrencyFilter;
            return `<button type="button" class="trade-strategy-dropdown-option${selected ? ' is-selected is-active' : ''}"
                            data-investment-currency-filter-option="${escapeHtml(value)}" role="option" aria-selected="${selected}">
                        <span class="trade-strategy-dropdown-check" aria-hidden="true"></span>
                        <span class="trade-strategy-dropdown-copy"><span class="trade-strategy-dropdown-title">${escapeHtml(label)}</span></span>
                    </button>`;
        }).join('');
    ...
    function mountInvestmentCurrencyFilterHeaders(root = document) {
        if (!investmentCurrencyFilterDocumentListenersBound) {
            investmentCurrencyFilterDocumentListenersBound = true;
            ...
            <div class="date-picker-field investment-stock-details-date-picker-field"
                ...
                <div class="date-picker-trigger" data-date-trigger aria-haspopup="dialog" aria-expanded="false">
                    <span class="date-picker-trigger-value"
                          ...
                          aria-describedby="${escapeHtml(feedbackId)}"
    ...
    function mountInvestmentStockDetailsTimeFilterHeaders(root = document) {
        if (!investmentStockDetailsTimeFilterDocumentListenersBound) {
            investmentStockDetailsTimeFilterDocumentListenersBound = true;
    ...
    function refreshInvestmentStockDetailsTableRows({ refreshHeaders = true } = {}) {
        if (!(investmentStockDetailsTableHost instanceof HTMLElement)) return;
        const activeTicker = ensureSelectedInvestmentStockTicker();
        if (!activeTicker) return;
        ...
        if (refreshHeaders) {
            ...
            mountInvestmentSideFilterHeaders(investmentStockDetailsTableHost);
            mountInvestmentCurrencyFilterHeaders(investmentStockDetailsTableHost);
        }
    ...
    function syncInvestmentImportMode() {
        ...
        const usesStandardXlsxImport = GENERIC_XLSX_INVESTMENT_BROKERS.has(selectedBroker);
        ...
        if (schwabPositionsCsvInput instanceof HTMLInputElement) {
            schwabPositionsCsvInput.required = isSchwab;
        }
    ...
    function renderInvestmentHistoryRowMarkup(txn) {
        ...
        const resolvedBrokerLabel = resolvedBinding?.targetTxn
            ...
            : 'counterpart account';
        const resolvedTransferPreposition = getInvestmentInternalTransferDirection(resolvedBinding?.sourceTxn) === 'security_broker_to_broker'
            ? 'to'
            : 'from';
    ...
    function getInvestmentBrokerFilteredRowsForVisibleHistory(processedTransactions = []) {
        if (activeInvestmentView !== 'metrics') {
            return getInvestmentBrokerFilteredTransactionRows(processedTransactions);
        }
        ...
        return selectInvestmentBrokerRows(
            ensureInvestmentBrokerFilterTransactionIndex(processedTransactions),
            selectedBrokerCodes,
        );
    }
    ...
    function buildInvestmentMarkdownExport() {
        ...
        if (activeInvestmentView === 'stock_details') {
            ...
            const historyHeaderTable = investmentStockDetailsTableHost?.querySelector('.investment-stock-details-table[data-table-header]');
    ...
    function getInvestmentHistoryScrollContainer() {
        return bindInvestmentScrollIntent(
            investmentHistorySurface?.querySelector('#history_table_wrap .investment-history-table-scroll'),
            'history',
        );
    }

    function getInvestmentHistoryTableBody() {
        return investmentHistorySurface?.querySelector('#history_table_wrap .investment-history-table-scroll #investment_history');
    }
    ...
    function getStockDetailLedgerNoForHistoryLedgerNo(ledgerNo, historyTicker = '') {
        ...
        const normalizedHistoryTicker = getInvestmentCanonicalTicker(historyTicker);
        if (
    ...
    function renderFundingMetricCards(
        ...
        return [
            renderMetricCards(OFFSHORE_GAIN_METRIC_DEFINITIONS, offshoreGainMetrics, { keyPrefix }),
            renderMetricCards(HOLDINGS_SUMMARY_METRIC_DEFINITIONS, holdingsSummaryMetrics, { keyPrefix }),
            renderMetricCards(FUNDING_METRIC_DEFINITIONS, fundingMetrics, { keyPrefix }),
            renderMetricCards(BROKER_BENEFIT_METRIC_DEFINITIONS, brokerBenefitMetrics, { keyPrefix }),
        ].join('');
    }
    ...
    function mapInvestmentMetricRowsToGlobalLedgerNos(metricValues, scopedTransactions) {
        ...
        const globalEntries = getSortedInvestmentMetricTransactions(investmentRawTransactionsCache);
        const globalLedgerNoByTransaction = new Map(
            globalEntries.map(({txn, ledgerNo}) => [txn, ledgerNo]),
        );
        const scopedEntries = getSortedInvestmentMetricTransactions(scopedTransactions);
    ...
    function getInvestmentBrokerSummaryTransactions(brokerCode = getInvestmentBrokerSummarySelectedCode()) {
        const normalizedBrokerCode = normalizeInvestmentBroker(brokerCode);
        if (normalizedBrokerCode === 'all') return [...investmentRawTransactionsCache];
        if (!normalizedBrokerCode) return [];
        return investmentRawTransactionsCache.filter((txn) => (
    ...
    function renderInvestmentMetricsPanel({
        ...
        const mergedLatestPrices = {
            ...(investmentBaseLatestPricesCache && typeof investmentBaseLatestPricesCache === 'object'
                ? investmentBaseLatestPricesCache
                : {}),
            ...(investmentLatestPricesCache && typeof investmentLatestPricesCache === 'object'
        ...
        const resolvedTotalEquity = isAllBrokersSelected && Number.isFinite(Number(totalEquity))
            ...
            : isAllBrokersSelected
                ? getLatestDashboardEquity(investmentProcessedTransactionsCache, investmentChartPointsCache)
        ...
        const resolvedBrokerBenefitMetrics = isAllBrokersSelected && brokerBenefitMetrics
            ? brokerBenefitMetrics
            : getBrokerBenefitMetrics(scopedTransactions, mergedLatestPrices, resolvedTotalEquity);
        ...
        const resolvedFundingMetrics = isAllBrokersSelected && fundingMetrics
            ? fundingMetrics
            : getUsdFundingMetrics(scopedTransactions);
    ...
    function syncImportValidationState() {
        ...
        const schwabTransactionsFile = schwabTransactionsCsvInput?.files?.[0];
        const schwabPositionsFile = schwabPositionsCsvInput?.files?.[0];
        const schwabTransactionsReady = isSchwab && isLikelyCsvFile(schwabTransactionsFile);
        const schwabPositionsReady = isSchwab && isLikelyCsvFile(schwabPositionsFile);
        ...
        setImportStatusIcon(schwabTransactionsCsvStatus, Boolean(schwabTransactionsReady));
        setImportStatusIcon(schwabPositionsCsvStatus, Boolean(schwabPositionsReady));
    ...
    if (investmentForm) {
        investmentForm.addEventListener('submit', (e) => {
            ...
            if (selectedBroker === 'ibkr') {
                ...
                const schwabTransactionsFile = schwabTransactionsCsvInput?.files?.[0] || transactionsCsv?.files?.[0];
                const schwabPositionsFile = schwabPositionsCsvInput?.files?.[0] || positionsCsv?.files?.[0];
                if (!schwabTransactionsFile || !schwabPositionsFile) {
                    setImportFeedback('Please choose both the Schwab Transactions CSV and Positions CSV before importing.', 'error');
                    return;
                }
                if (!isLikelyCsvFile(schwabTransactionsFile) || !isLikelyCsvFile(schwabPositionsFile)) {
                    setImportFeedback('Please upload the Schwab Transactions and Positions exports as CSV files.', 'error');
                    return;
                }
                formData.append('transactions_csv', schwabTransactionsFile);
                formData.append('positions_csv', schwabPositionsFile);
            } else if (GENERIC_XLSX_INVESTMENT_BROKERS.has(selectedBroker)) {
    ...
    function renderHoldingsTable(
        ...
        const rowsHtml = summaries.map((summary, index) => {
            ...
            const moneyMarketFundTokenLogoClass = getMoneyMarketFundTokenLogoClass(summary.ticker);
    ...
    function renderInvestmentStockDetailsPanel(tickerProfiles = {}) {
        ...
        const moneyMarketFundTokenLogoClass = getMoneyMarketFundTokenLogoClass(activeTicker);
        ...
        if (investmentStockDetailsTableHost instanceof HTMLElement) {
            investmentStockDetailsTableHost.innerHTML = `
                <div class="scrollable-data-table-shell investment-history-table-shell investment-stock-details-table-shell">
                    ...
                    <div class="trade-transactions-wrap scrollable-data-table-scroll investment-history-table-scroll investment-stock-details-table-scroll" data-table-scroll>
            ...
            mountInvestmentSideFilterHeaders(investmentStockDetailsTableHost);
            mountInvestmentCurrencyFilterHeaders(investmentStockDetailsTableHost);
            mountInvestmentStockDetailsTimeFilterHeaders(investmentStockDetailsTableHost);
            bindStockDetailsHistoryInteractions(investmentStockDetailsTableHost);
            syncInvestmentStockDetailsTableVisibility();
        }
    ...
    function bindInvestmentHistoryChartInteractions(historyContainer) {
        ...
        historyContainer.querySelectorAll('tr[data-investment-history-row]').forEach((row) => {
            ...
            const activateChartMarker = () => {
                ...
                const stockDetailLedgerNo = getStockDetailLedgerNoForHistoryLedgerNo(
                    ledgerNo,
    ...
    function getVisibleInvestmentHistoryTransactions(processedTransactions = [], chartPoints = []) {
        return selectVisibleInvestmentHistoryTransactions({
            brokerFilteredRows: getInvestmentBrokerFilteredRowsForVisibleHistory(processedTransactions),
            chartPoints,
    ...
    async function renderTransactionTable(transactions, { preserveHistoryPage = false, scrollToTop = true } = {}) {
        ...
        function applyHoldingStateUpdate(state, txn, normalizedType, valuationQty, price) {
            if (isLongbridgeHkCashEquivalentTransfer(txn)) {
                const syntheticSourceTicker = getLongbridgeHkCashEquivalentSyntheticTicker(txn);
                const syntheticTicker = getInvestmentCanonicalTicker(syntheticSourceTicker);
    ...
    function formatInvestmentMetricTooltipRowLabel(entry) {
        ...
        const description = String(formatTransactionDescription(entry.txn) || '').replace(/\s+/g, ' ').trim();
    ...
    function renderMetricValueWithTooltip(metric, { keyPrefix = '' } = {}) {
                    ...
                    ${visibleRows.map((rowNo) => `
                        <li>
                            <span class="investment-metric-tooltip-list-line">
                                <span class="investment-metric-tooltip-row-no">${escapeHtml(String(rowNo))}</span>
                                <span class="investment-metric-tooltip-row-copy">
                                    <span class="investment-metric-tooltip-row-date">${escapeHtml(formatTooltipLedgerDate(ledgerEntryMap.get(rowNo)?.date) || 'Date unavailable')}</span>
                                    <span class="investment-metric-tooltip-row-description">${escapeHtml(formatInvestmentMetricTooltipRowLabel(ledgerEntryMap.get(rowNo)))}</span>
                                </span>
                            </span>
                        </li>
                    `).join('')}
        ...
        return `
            <span class="investment-metric-tooltip-trigger trade-metric-value${valueClass ? ` ${valueClass}` : ''}" tabindex="0" data-metric-key="${escapeHtml(metricKey)}" data-metric-target-row="${escapeHtml(String(latestRow))}" data-workspace-mask="trade-metric">
                ...
                <span id="${tooltipId}" class="investment-metric-tooltip field-tooltip liquid-glass-surface" role="tooltip" aria-hidden="true" data-investment-metric-tooltip="1">
                    <span class="investment-metric-tooltip-section">
                        ...
                        <span class="investment-metric-tooltip-copy">${escapeHtml(metric?.summary || '')}</span>
                    </span>
                    <span class="investment-metric-tooltip-section">
    ...
    function closeInvestmentMetricTooltip({ clearHistory = true } = {}) {
        const activeState = activeInvestmentMetricTooltipState;
        if (!activeState) {
            if (clearHistory) clearInvestmentHistoryHighlights();
            return;
        }
        ...
        activeInvestmentMetricTooltipState = null;
        if (clearHistory) clearInvestmentHistoryHighlights();
    }
    ...
    function bindInvestmentMetricTooltipInteractions(metricsPanel) {
        ...
        metricsPanel.querySelectorAll('.investment-metric-tooltip-trigger').forEach((trigger) => {
            ...
            const openTooltip = () => {
                closeInvestmentMetricTooltip({ clearHistory: false });
                activeInvestmentMetricTooltipState = {
                    trigger,
                    tooltip,
                ...
                window.addEventListener('resize', activeInvestmentMetricTooltipState.reposition);
                window.addEventListener('scroll', activeInvestmentMetricTooltipState.reposition, true);
            };
            const closeTooltip = () => {
                if (activeInvestmentMetricTooltipState?.tooltip !== tooltip) return;
